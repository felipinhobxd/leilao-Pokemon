...
insert into public.whatsapp_groups(group_jid,name,active,is_default) values('120000000000000000@g.us','Grupo teste',true,true) returning id \gset
...
declare payload jsonb; result jsonb; replay jsonb; aid uuid; begin
 ...
 perform pg_temp.check_that((select poll_title='16. Lances' and group_id=:'id'::uuid from public.whatsapp_dispatches where auction_id=aid),'dispatch uses selected group');
 begin
