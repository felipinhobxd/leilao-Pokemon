...
-- permanently deleted. The auction deletion is the same FK-safe operation as
-- before; the only addition is queue bookkeeping after the dispatch is removed.
begin;
...
declare
  ...
  deleted_queue_id uuid;
  deleted_queue_status text;
  deleted_queue_total integer;
  remaining_queue_pending integer;
begin
  ...
  select dd.queue_id into deleted_queue_id
    from public.whatsapp_dispatches dd
   where dd.auction_id=a.id
   limit 1;
  ...
  if deleted_queue_id is not null then
    ...
    update public.auction_publish_queues q
       set total_items=greatest(q.total_items-1,0),
           status=case
             when remaining_queue_pending=0 and q.status not in('cancelled','completed') then 'completed'
             else q.status
           end,
