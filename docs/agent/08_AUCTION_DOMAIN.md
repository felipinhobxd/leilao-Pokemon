...
- **Aviso global (`participant_warnings`)**: cada `BID_CHANGED` com redução = 1 aviso no contador do USUÁRIO (vale no sistema inteiro, nunca reseta, um por evento por idempotência). Exatamente 3 → `admin_notifications` → DM aos admins (formato em `bot/warning-notify.mjs`). **DM ao PARTICIPANTE (20260924180000)**: cada redução também dispara DM direto à pessoa com qual enquete/lote, valores e horário + "aviso K de 3" (`participant_warnings.notified_at`). Subir/manter valor não gera aviso. O dashboard mostra a tabela ao vivo ("Avisos de alteração de valores") e o Excel mantém as abas dedicadas.
