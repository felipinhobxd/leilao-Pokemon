...
type Draft = {
  id: string; file: File | null; preview: string; imageUrl: string;
...
function draftFor(file: File | null, preview: string, lot: number, expanded: boolean): Draft {
  return {
    id: crypto.randomUUID(), file, preview, imageUrl: "", name: file ? baseName(file.name) : "", collection: "", cardNumber: "",
...
async function optimize(file: File) {
  ...
  try {
    ...
    return new File([blob], `${baseName(file.name) || "carta"}.webp`, { type: "image/webp", lastModified: file.lastModified });
  } catch { return file; }
}
...
export default function BulkAuctionWizard() {
  ...
  async function uploadImages() {
    const urls = new Map<string, string>(), localDedup = new Map<string, string>();
    for (let index = 0; index < cards.length; index++) {
      const card = cards[index];
      if (!card.file) { urls.set(card.id, card.imageUrl.trim()); continue; }
      setUploadProgress(`Imagem ${index + 1} de ${cards.length}…`);
      const hash = await fingerprint(card.file), cached = localDedup.get(hash);
      if (cached) { urls.set(card.id, cached); continue; }
      const optimized = await optimize(card.file);
      if (optimized.size > 5 * 1024 * 1024) throw new Error(`Carta ${index + 1}: imagem maior que 5 MB mesmo após otimização.`);
      const form = new FormData(); form.set("file", optimized);
      const response = await authFetch("/api/cards/image", { method: "POST", body: form });
      const body = await response.json();
      if (!response.ok) throw new Error(`Carta ${index + 1}: ${body.error ?? "falha no upload."}`);
      const url = String(body.url ?? ""); if (!url.startsWith("https://")) throw new Error(`Carta ${index + 1}: upload inválido.`);
      localDedup.set(hash, url); urls.set(card.id, url);
    }
    return urls;
  }
