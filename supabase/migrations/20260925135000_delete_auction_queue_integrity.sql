  ...
  if deleted_queue_id is not null then
    select
      ...
      + (select count(*) from public.whatsapp_quick_polls qp
        where qp.queue_id=deleted_queue_id)
      into remaining_queue_items;

    if remaining_queue_items=0 then
      ...
      update public.auction_publish_queues q
         set total_items=remaining_queue_items,
             status=case
