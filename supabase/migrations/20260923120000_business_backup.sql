...
returns jsonb language sql stable security invoker set search_path='' as $body$
 select jsonb_build_object(
  ...
  'participant_identities', coalesce((select jsonb_agg(t order by id) from public.participant_identities t),'[]'::jsonb),
  ...
  'processed_commands', coalesce((select jsonb_agg(t order by id) from public.processed_commands t),'[]'::jsonb),
  ...
  'whatsapp_vote_state', coalesce((select jsonb_agg(t order by id) from public.whatsapp_vote_state t),'[]'::jsonb)
 );
$body$;
