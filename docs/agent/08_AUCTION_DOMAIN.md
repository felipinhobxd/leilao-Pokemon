...
- **Vencedor**: `AUCTION_FINALIZE` no vencimento — maior lance ativo de participante elegível; **tie-break pelo horário REAL do voto** (`whatsapp_event_at`), depois `processed_at`, depois `confirmation_order`. Sem lances → `closed` e carta volta a `available`.
