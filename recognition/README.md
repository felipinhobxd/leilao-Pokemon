...
- **URL de imagem utilizável**: candidatos resolvidos via `low.webp`/espelho EN não anunciam mais o `high.webp` do CDN (404) — o serviço aponta para o endpoint local `/scan/{language}/{cardId}`, que serve o que realmente resolveu (incluindo cartas resolvidas só via `image_alt` de 2ª/3ª fonte — o endpoint aceita registros sem `image_base`).
- **Regra de negócio pt-BR pré-2011 (language-conflict)**: o catálogo pt-BR começa em 2011 (bw1); impressões Devir (EX/Neo, 1999–2010) têm apenas a gêmea EN no catálogo. Quando o OCR lê português com confiança forte (≥0,75, ≥2 palavras características) e o vencedor é de OUTRO idioma sem nenhuma gêmea pt-BR no pool, a identidade se mantém como melhor candidato mas a decisão é rebaixada a REVISAR com `languageStatus: "conflict"` + evidência `language-conflict` — nunca um IDENTIFICADO/PROVÁVEL na impressão errada. Leitura fraca (palavra compartilhada, 0,62) nunca dispara o conflito.
- **Foto deitada (90°/270°)**: quad paisagem é warpado para 840×600 e então rotacionado CCW — a saída volta SEMPRE como retrato 600×840 com aspecto preservado. Antes, o warp fixo 600×840 + rotação devolvia uma carta 840×600 com aspecto invertido (~2× de distorção), quebrando regiões de OCR, retrieval SigLIP e sondas SIFT para toda foto deitada.
- **Orçamento interativo de scan**: o caminho de requisição (`/recognize`, `/scan`) resolve scans com `fast=True` — 1 tentativa curta por URL em vez da escada de sync (4 tentativas, backoff 5/15/45/120s). O cache negativo com TTL por classe mantém a porta aberta: os caminhos de sync/backgound continuam com a escada agressiva e curam o gap no próximo run.
...
- `get_scan_state` agora retorna `(state, bytes, sha256, source, updated_at, validated_mtime)` — os índices 0-3 inalterados; o 5º campo é a chave do re-run stat-only (abaixo).

**Re-runs stat-only (Fase 5, validado ao vivo: 5,2s para 57.148 cartas)**
- O re-hash completo de ~5,6 GB de scans a cada execução (190s+) violava o contrato "zero re-hashing" do fast-path. Cada linha `validated` agora grava o `validated_mtime` do arquivo cujos bytes o sha256 cobre; um re-run que vê o MESMO size + mtime pula o hash (stat-only, ~11.000 cartas/s com 12 workers). Qualquer mudança de mtime/size cai na verificação sha completa (healing preservado); linhas legadas re-hasham uma única vez e são regravadas com a chave (self-healing).
- O caminho EXATO do arquivo é derivado do `source` gravado (qualidade/passo da cadeia) — 1 stat por carta em vez de sondar a cadeia toda; a sonda continua como fallback.
- **Fix Windows real**: `_ptcg_cache_path` em URLs de hosts estrangeiros (promos de 30º aniversário que o TCGdex publica em `images.scrydex.com`) gerava filename com `:` — inválido no Windows (OSError 87) e as 3 cartas contavam como `failed` sem nem chegar à rede. Sanitização de caracteres proibidos + teto de comprimento + sufixo `.img` para URLs sem extensão. As 3 cartas agora baixam e validam de verdade.

Regressões obrigatórias: `tests/test_units.py::TestScanDecode` (WebP ~2 KB, WebP alfa, GIF/JPEG/PNG, truncado), `tests/test_catalog_round.py::TestScanFastPath` (re-run com zero rede nos 3 estados, mudança de escopo, healing, stat-only mtime) e `TestSqliteConcurrency` (12 workers sem `database is locked`).
...
**Fase 4 — concorrência, segurança e integridade dos lotes**
- **4.1 SQLite thread-local** (`download_scans`/`catalog`): cada worker tem a PRÓPRIA conexão (`_thread_conn()` sobre `threading.local()`) em vez da conexão única compartilhada com `check_same_thread=False`; WAL continua como mecanismo de concorrência e `PRAGMA busy_timeout=5000` faz a contenção esperar em vez de estourar `database is locked`. Causa raiz encontrada com stress test de 12 threads: (1) o `init_db` deixava a transação de escrita do backfill de `canonical_id` ABERTA (nunca commitada) — cada conexão saía segurando seu write-lock até o primeiro commit do caller; (2) cursores de SELECT de linha única não fechados seguravam snapshot de leitura, e o upgrade leitura→escrita é o único caminho SQLite que NÃO respeita busy_timeout. Fixes: `commit()` no fim do `init_db`, guarda `EXISTS` no backfill (caso comum fica read-only) e fechamento explícito de cursores em `get_scan_state`/`scan_state_counts`/`get_meta`. O RLock fica APENAS para migrações/metas (`init_db`) e caminhos batch do reconciler. Stress test: 12 workers × 25 escritas + leituras concorrentes, zero erros, <1s (antes: 66s de timeouts).
- **4.3 Sem lotes "fantasmas"** (`/catalog/exists` + `find_catalog_card`): o wizard consulta o serviço local antes de criar o lote — carta (idioma, coleção-id-ou-nome, número) precisa existir no catálogo; matching tolerante do jeito que o usuário digita (padding de zeros, `SWSH074`/`TG05` exatos, `SVP-001`↔`001` com guarda de prefixo) mas NUNCA fuzzy (set errado falha). Deploy fora do PC do bot (Vercel não vê o loopback): lote criado sem validação + log visível; `RECOGNITION_STRICT_CATALOG=1` ativa o modo bloqueante para quem roda tudo na mesma máquina. Endpoint leve por desenho: SQLite puro, sem modelos, sem warm-up.
- **4.2 Rate limit no `/api/cards/image/authorize`** (web-tier): 50 requisições/min por usuário com 429 + `Retry-After`; Redis distribuído quando `REDIS_URL` existe, fallback in-memory documentado (1 instância). Sem isso, um único usuário esgota a cota de `signedUrl` do Supabase.
- Regressões: `test_units.py::TestSqliteConcurrency` (12 workers, busy_timeout), `test_catalog_round.py::TestFindCatalogCard` (padding, alfanuméricos, tail, recusas), `tests/rate-limit.test.mjs`, `tests/card-catalog.test.mjs`.
